...
## Read table into one variable from "data" specifyingthe separator character
...
## set the graphical parameters for the plots 
## i. e. two rows and two columns
par(mfrow =c(2, 2))
...
plot(power1$Date_Time, power1$Global_active_power, 
     ylab = "Global Active Power",  ## y label name
...
## plot for Voltage
plot(power1$Date_Time, power1$Voltage, 
     ylab = "Voltage",  ## y label name
     xlab = "datetime", ## no x label name
     type = "l" ## linear type
)

## -- PLOT Bottom Left -- 

## plot data for first sub metering
plot(power1$Date_Time, power1$Sub_metering_1,
     ylab = "Energy sub metering",  ## y label name
     xlab = "", ## no x label name
     type = "l" ## linear type
)

## add sub metering two and three to the plot
lines(power1$Date_Time, power1$Sub_metering_2, col = "red")
lines(power1$Date_Time, power1$Sub_metering_3, col = "blue")

## add legend to the plot
legend("topright", ## co-ordinates for the legend box
       bty = "n", ## no border as in example
       legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), ## values to appear
       lty = 1, ## line type and widht
       col = c("black", "red", "blue") ## color of lines appearing in the legend
       )

## -- PLOT Bottom Right 

## plot for Global Reactive Power
plot(power1$Date_Time, power1$Global_reactive_power, 
     ylab = "Global_reactive_power",  ## y label name
     xlab = "datetime", ## no x label name
     type = "l" ## linear type
)
